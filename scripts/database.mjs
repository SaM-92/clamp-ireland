...
const usage = "Use init, accounts, or admins <first UUID> <second UUID>. "
  ...
  if (command === "accounts") {
    ...
    await transaction(pool, async (tx) => {
      for (const id of args) {
        if (!(await tx.prepare("SELECT id FROM profiles WHERE id=? AND is_banned=0").get(id))) throw new Error("Both accounts must first sign in with Google and must not be banned.");
      }
