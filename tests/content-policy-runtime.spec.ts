...
import { sqlRuntime, owner, cofounder, locationId, publicOrigin } from "./helpers/sql-runtime";
import { POLICY_TEXT } from "../src/modules/content-policy/policy";
import { PHOTO_LIMITS } from "../src/modules/photos/policy";
import { transaction } from "../database/sql-store.mjs";
...
async function fixture() {
  ...
  const f = await sqlRuntime({
    ...
    "@/lib/db/server": {
      database: async () => f.db,
      writeTransaction: async <T>(work: (db: SqlConnection) => Promise<T>) => {
        const result = await transaction(f.pool, work);
        if (state.uncertain && (await f.db.prepare("SELECT count(*) AS total FROM reports").get())?.total) {
...
test("cookie authentication and bounded input precede inference, upload and report creation", async () => {
  ...
  try {
    ...
    expect((await f.reports.POST(await f.report("x".repeat(2001)))).status).toBe(400);
    for (const body of [{ username: "valid_name", userId: "injected" }, {}, null, { username: "a".repeat(25) }]) {
      expect((await f.usernames.PUT(await f.username(body))).status).toBe(400);
    }
    expect(f.state.aiCalls).toBe(0);
    expect(f.state.uploads).toBe(0);
    expect((await f.db.prepare("SELECT count(*) AS total FROM content_policy_limits").get())?.total).toBe(0);
    expect((await f.db.prepare("SELECT count(*) AS total FROM reports").get())?.total).toBe(0);
...
test("policy rejection, malformed AI output and failures never upload or bypass paid-request reservations", async () => {
  const f = await fixture();
  try {
    const cheap = await f.reports.POST(await f.report("fuck"));
    ...
    expect((await f.reports.POST(await f.report())).status).toBe(422);
    f.state.decisions = { decision: "approve", code: "allowed", injected: true };
    expect((await f.reports.POST(await f.report())).status).toBe(503);
    f.state.offline = true;
    const failed = await f.reports.POST(await f.report());
    expect(failed.status).toBe(503);
    expect(await failed.text()).not.toContain("PRIVATE");
    expect((await f.db.prepare("SELECT attempts FROM content_policy_limits WHERE scope='global'").get())?.attempts).toBe(3);
    expect(f.state.uploads).toBe(0);
    expect((await f.db.prepare("SELECT count(*) AS total FROM reports").get())?.total).toBe(0);
  } finally { await f.close(); }
});
...
test("approved normalized notes remain pending and forged approval objects cannot insert or rename", async () => {
  const f = await fixture();
  try {
    const response = await f.reports.POST(await f.report("  The fee was unfair.\nPoor service. "));
    ...
    expect(await f.db.prepare("SELECT description_raw,moderation_status,reviewed_at FROM reports").get()).toEqual({
    ...
    await expect(async () => profiles.savePublicIdentity(owner, { kind: "username", text: "forged_name" })).rejects.toThrow();
    expect((await f.db.prepare("SELECT count(*) AS total FROM reports").get())?.total).toBe(1);
  } finally { await f.close(); }
});
...
test("checked pseudonym onboarding is mandatory, unique and excludes Google metadata", async () => {
  const f = await fixture();
  try {
    await f.db.prepare("UPDATE profiles SET display_name=NULL,username_policy_checked_at=NULL WHERE id=?").run(owner);
    expect(await (await f.usernames.GET(await f.request("/api/profile/username"))).json()).toEqual({ username: null, needsOnboarding: true });
    expect((await f.reports.POST(await f.report())).status).toBe(409);
    expect(f.state.aiCalls).toBe(0);
    expect(await (await f.usernames.PUT(await f.username({ username: "River_Trail" }))).json()).toEqual({ username: "river_trail", needsOnboarding: false });
    expect((await f.usernames.PUT(await f.username({ username: "park_walker" }))).status).toBe(409);
    expect((await f.usernames.PUT(await f.username({ username: "admin" }))).status).toBe(422);
    expect((await f.db.prepare("SELECT display_name FROM profiles WHERE id=?").get(cofounder))?.display_name).toBe("park_walker");
    const calls = f.state.aiCalls;
    await f.db.prepare("UPDATE profiles SET is_banned=1 WHERE id=?").run(owner);
    expect((await f.reports.POST(await f.report())).status).toBe(401);
    expect((await f.usernames.PUT(await f.username())).status).toBe(401);
...
test("quota exhaustion and unavailable quota storage stop both mutation paths", async () => {
  ...
  try {
    ...
    expect((await f.reports.POST(await f.report())).status).toBe(429);
    expect((await f.usernames.PUT(await f.username())).status).toBe(429);
    await f.db.exec("DROP TABLE content_policy_limits");
    expect((await f.reports.POST(await f.report())).status).toBe(503);
    expect((await f.usernames.PUT(await f.username())).status).toBe(503);
    expect(f.state.aiCalls).toBe(0);
    expect(f.state.uploads).toBe(0);
  } finally { await f.close(); }
});
...
test("50 MiB image and streamed body boundaries apply even when content length lies", async () => {
  ...
  try {
    ...
    const name = async (bytes: number) => f.usernames.PUT(await f.request("/api/profile/username", { method: "PUT", body: json.padEnd(bytes) }));
    ...
    expect((await f.reports.POST(await f.report("A factual note.", {}, PHOTO_LIMITS.sourceBytes + 1))).status).toBe(413);
    expect((await f.reports.POST(await f.report("A factual note.", {}, 10, "text/plain"))).status).toBe(400);
...
test("failed insert cleanup only deletes confirmed rollbacks and never uncertain committed evidence", async () => {
  const f = await fixture();
  try {
    await f.db.exec("CREATE TRIGGER reject_insert ON reports INSTEAD OF INSERT AS BEGIN RAISERROR('fixture insert failure',16,1); END");
    expect((await f.reports.POST(await f.report())).status).toBe(503);
    ...
    await f.db.exec("DROP TRIGGER reject_insert");
...
test("report admission serializes bodies and releases after a failed inference", async () => {
  ...
  try {
    ...
    const first = f.reports.POST(await f.report());
    await entered.promise;
    const second = await f.reports.POST(await f.report());
    expect(second.status).toBe(429);
    expect(second.headers.get("retry-after")).toBe("10");
    f.state.offline = true; resume.resolve();
    expect((await first).status).toBe(503);
    f.state.offline = false;
    expect((await f.reports.POST(await f.report())).status).toBe(200);
  } finally { await f.close(); }
});
