...
test("WGS84 summaries include 499/500 metres, exclude 501 and do not chain neighbouring circles", async () => {
  ...
  try {
    ...
    for (const metres of [499, 500, 501, 999]) {
      ...
      await f.db.prepare("INSERT INTO locations(id,lat,lng,created_at,updated_at) VALUES (?,?,?,?,?)").run(id, point.lat2, point.lon2, now, now);
      await f.report({ location: id, text: `Parking ${metres}` });
    }
...
test("approval is atomic, current and human-only; votes never invalidate summaries", async () => {
  ...
  try {
    ...
    await expect(async () => repo.approveAreaSummaryDraft(id, outsider, "Reports mention signs.", fingerprint)).rejects.toThrow("administrator");
    expect(await f.db.prepare("SELECT sentence,status FROM area_summaries WHERE id=?").get(id)).toMatchObject({ sentence, status: "draft" });
    ...
    await f.db.prepare("UPDATE reports SET description='Changed wording' WHERE id=?").run(report);
    await f.db.prepare("UPDATE reports SET description='Parking permits are mentioned.' WHERE id=?").run(report);
    await repo.invalidateNearbyAreaSummaries(spot, f.db);
    expect(await repo.getPublicAreaSummary(spot)).toBeNull();
    expect((await f.db.prepare("SELECT status FROM area_summaries WHERE id=?").get(id))?.status).toBe("stale");
    await expect(async () => repo.approveAreaSummaryDraft(regenerated, owner, sentence, fingerprint)).rejects.toThrow("existing draft");
  } finally { await f.close(); }
});
...
test("source membership, timestamps, geometry and deletion permanently stale nearby summaries", async () => {
  for (const change of ["insert", "remove", "reviewed", "move", "delete"] as const) {
    ...
    try {
      ...
      if (change === "insert") await f.report();
      if (change === "remove") await f.db.prepare("UPDATE reports SET is_removed=1 WHERE id=?").run(report);
      if (change === "reviewed") await f.db.prepare("UPDATE reports SET reviewed_at='2026-01-01T00:00:00Z' WHERE id=?").run(report);
      if (change === "move") await f.db.prepare("UPDATE locations SET lat=53.4 WHERE id=?").run(locationId);
      if (change === "delete") await f.db.prepare("DELETE FROM locations WHERE id=?").run(locationId);
      // Every write path that changes a source's facts must invalidate nearby
      // summaries itself (no DB trigger can do geodesic math in Azure SQL);
      // this mirrors what approveReport/future edit-or-move paths must call.
      await repo.invalidateNearbyAreaSummaries(spot, f.db);
      expect((await f.db.prepare("SELECT status FROM area_summaries WHERE id=?").get(id))?.status).toBe("stale");
      expect(await repo.getPublicAreaSummary(spot)).toBeNull();
    } finally { await f.close(); }
...
test("complete source limits, validated wording, stale fingerprints and lease cooldown fail explicitly", async () => {
  ...
  try {
    const repo = f.load<typeof import("../src/modules/area-summaries/server/repository")>("src/modules/area-summaries/server/repository.ts");
    await expect(async () => repo.getAreaSummarySources(spot)).rejects.toThrow("No human-approved");
    await f.report();
    const fp = (await repo.getAreaSummarySources(spot)).source_fingerprint;
    await expect(async () => repo.saveAreaSummaryDraft(spot, "0".repeat(64), { sentence })).rejects.toThrow("sources changed");
    await expect(async () => repo.saveAreaSummaryDraft(spot, fp, { sentence, approved: true })).rejects.toThrow();
    await expect(async () => repo.saveAreaSummaryDraft(spot, fp, { sentence: "Reports mention " + "parking ".repeat(21) + "." })).rejects.toThrow();
    const lease = await repo.acquireAreaSummaryGeneration(spot);
    await expect(async () => repo.acquireAreaSummaryGeneration(spot)).rejects.toMatchObject({ status: 429 });
    await repo.releaseAreaSummaryGeneration(lease);
    await expect(async () => repo.acquireAreaSummaryGeneration(spot)).rejects.toMatchObject({ status: 429 });
    await f.db.prepare("UPDATE area_summary_generation_leases SET expires_at=0").run();
    expect(await repo.acquireAreaSummaryGeneration(spot)).not.toBe(lease);
    for (let n = 0; n < 200; n++) await f.report();
    expect((await repo.getAreaSummarySourceState(spot)).source_count).toBe(201);
    await expect(async () => repo.getAreaSummarySources(spot)).rejects.toThrow("resource limit");
    await f.db.prepare("DELETE FROM reports").run();
    for (let n = 0; n < 25; n++) await f.report({ text: "é".repeat(1000) });
