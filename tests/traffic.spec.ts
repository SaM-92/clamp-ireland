...
test("traffic reads stay protected and local collection is clearly disabled", async ({ request }) => {
  expect((await request.get("/api/admin/traffic")).status()).toBe(404);
  expect((await request.get(adminUrl("/api/admin/traffic"))).status()).toBe(403);
...
test("375px private dashboard explains disabled analytics without inventing counts or collecting traffic", async ({ page }) => {
  await page.setViewportSize({ width: 375, height: 812 });
  const requests: string[] = [];
  page.on("request", (request) => {
    if (/\/api\/analytics/.test(request.url())) requests.push(request.url());
  });
