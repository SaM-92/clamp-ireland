...
`/auth/username` is a working private/noindex account form, reached after password sign-in or the Google callback and through the account menu. It supports initial setup and later changes, shows policy/service errors, and offers browsing without posting. Email signup still requires confirmation first. Google OAuth redirect configuration must eventually permit `/auth/username`; **no remote auth configuration was changed here**. Existing accounts retain their IDs, privileges and report ownership; unapproved legacy placeholders/names are cleared by the migration and shown as pending onboarding, not leaked or presumed approved.
