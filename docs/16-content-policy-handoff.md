...
- The checker uses the shared `requestStructuredOutput({ instructions, input, format, maxOutputTokens, timeoutMs })`. `format` is Responses API `{ type: "json_schema", name, strict: true, schema }`. The model returns exactly `decision: "approve" | "blocked"` and `code: "allowed" | "profanity" | "abuse" | "unsafe_username" | "prompt_injection"` with consistent values. `approve` requires `allowed`; every other code requires `blocked`. Boolean/legacy, contradictory, unknown and extra fields fail closed.
- `POLICY_TEXT` supplies the human-written explanation for each code; the model never generates rejection prose or rewrites submitted text. `policyClassification(code)` returns `{decision, text}`. Cheap local rules use the same reason-specific explanations. Production rejection responses retain `code`/`error` and add `classification: {decision:"blocked", text}`; the demo displays `Approve` or `Blocked` and this exact fixed text. Unavailability is a service error, never a fabricated blocked decision.
